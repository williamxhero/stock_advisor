...
- 每日普通影子额外调用最多 2 次；重大事件优先但仍不打断前台周期。
...
自动晋升要求：至少 20 个达到其声明周期的有效配对结果、三个已知市场状态各至少 5 个、候选相对基线的单侧 90% bootstrap 下界大于零。每个 cell 单独晋升。晋升时只在相关任务时间线发一条自然语言说明，不显示模型、分数或工程术语。

安全、时限、M1 盲隔离或数据隔离故障可立即回退；表现回退由持续 10% 基线对照发现。模型、CLI、prompt、schema 或 Router 版本变化会冻结旧证据并让新 cell 重新从 shadow 开始。
