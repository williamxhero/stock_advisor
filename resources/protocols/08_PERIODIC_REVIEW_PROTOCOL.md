...
- v1.2（2026-08-24）：所有有效周期任务写入本地本地运行时数据库并投递AI交易伙伴，统一正文、摘要与状态语义。
...
1. `resources/governance/00_PROJECT_GUIDE.md`；
2. `workspace/portfolio/01_CURRENT_PORTFOLIO.md`；
3. `resources/knowledge/02_TRADING_PLAYBOOK.md`；
4. `resources/knowledge/03_CASEBOOK.md`；
5. `resources/knowledge/04_HYPOTHESES.md`；
6. `workspace/logs/05_DECISION_LOG.csv`；
7. 本文件；
8. `resources/protocols/09_OPPORTUNITY_DISCOVERY_PROTOCOL.md`；
9. `workspace/state/10_THEME_STATE.csv`；
10. `workspace/state/11_STOCK_STATE.csv`；
11. `workspace/logs/12_OPPORTUNITY_LOG.csv`；
12. `resources/governance/13_DATA_SEMANTICS.md`；
13. 对应周期内已有的月度、季度、年度总结。

事实冲突按：

> 用户最新明确真实成交/资产信息 > `workspace/portfolio/01_CURRENT_PORTFOLIO.md` > `workspace/logs/05_DECISION_LOG.csv`正式记录 > Project其他文件 > 历史聊天 > 普通Memory
...
- 可按周期原地维护`resources/knowledge/03_CASEBOOK.md`、`resources/knowledge/04_HYPOTHESES.md`、`workspace/logs/05_DECISION_LOG.csv`；禁止创建同名副本。
- `workspace/portfolio/01_CURRENT_PORTFOLIO.md`默认禁止修改；只有用户在当前聊天明确报告尚未写入且信息完整的真实成交时才可更新。
- 不得把研究结论、建议或推测写成真实成交。
- 不得改写Decision Log历史样本的`protocol_version`；周期报告记录自身`PeriodicReview-v1.2`，并比较不同Daily Execution与Opportunity Discovery版本。
- `workspace/state/10_THEME_STATE.csv`和`workspace/state/11_STOCK_STATE.csv`是当前分析状态；周期复盘可以校正当前状态，但不得删除或改写`workspace/logs/12_OPPORTUNITY_LOG.csv`中的历史发现、拒绝和当时证据。
- 文件身份不明、版本冲突或写入失败时停止对应写入并报告，不得创建替代副本。

### 1.7 Playbook变更门槛

- `resources/knowledge/02_TRADING_PLAYBOOK.md`默认只读。
...
- 所有有效周期任务由本地运行时原子保存报告、状态和事件；桌面端从 Exchange 投影读取，不再向 Codex thread 投递。
