...
PORTFOLIO_RELATIVE = Path("data/portfolio/01_CURRENT_PORTFOLIO.md")
DECISION_LOG_RELATIVE = Path("data/logs/05_DECISION_LOG.csv")
# Internal instrument resolution only. It must not be projected as an "AI 状态"
# in the holdings window.
STATE_RELATIVE = Path("data/state/11_STOCK_STATE.csv")
...
class PortfolioService:
    ...
    def __init__(self, project_root: Path, store: Any) -> None:
    ...
    def _render_portfolio(self) -> None:
        ...
        marker = "## AI Decision Center 成交记录"
    ...
    def _transaction_markdown(self) -> str:
        ...
        lines = ["## AI Decision Center 成交记录", "", "> 以下记录由用户明确成交陈述生成；撤销以反向事件保留历史。", ""]
        if not rows:
            return "\n".join(lines + ["- 暂无新增成交。"])
        for row in rows:
