...
class CompanionStore:
    ...
    def create_cycle(self, task_key: str, scheduled_for: str, as_of: str) -> dict[str, Any]:
            ...
            if claimed:
                ...
                if existing:
                    ...
                    c.execute(
                        ...
                        (cycle_id, task_key, scheduled_for, as_of, at, at),
                    )
    ...
    def transition(self, cycle_id: str, state: str, **fields: Any) -> dict[str, Any]:
        allowed={
            "human_deadline","voice_grace_deadline","m0_revealed_at","codex_session_id","packet_hash",
    ...
    def append_artifact(self, cycle_id: str, kind: str, actor: str, body: str, as_of: str, metadata: dict[str, Any] | None=None, *, occurred_at: str | None=None, known_at: str | None=None) -> dict[str, Any]:
        ...
        with self.connection() as c:
            ...
            if kind in {"h0", "m1", "m2", "outcome", "reflection"}:
                c.execute(
    ...
    def finish_attempt(self, attempt_id: str, status: str, *, output_sha256: str | None = None, error: str | None = None, usage: dict[str, Any] | None = None, verifier: dict[str, Any] | None = None) -> None:
        ...
        with self.connection() as c:
            c.execute(
                ...
                (status, now(), output_sha256, error[-2000:] if error else None,
                 ...
                 json.dumps(verifier or {}, ensure_ascii=False, sort_keys=True), now(), attempt_id),
            )
    ...
    def queue_router_shadow(self, decision_id: str, cycle_id: str, stage: str, packet: dict[str, Any], schema_name: str, candidate: dict[str, Any], *, priority: int = 0) -> str | None:
        ...
        with self.connection() as c:
            ...
            # Fixed daily budget: at most two extra candidate executions, with all major routes admitted.
            if priority <= 0:
                count = c.execute("SELECT COUNT(*) FROM router_shadow_job WHERE substr(created_at,1,10)=substr(?,1,10)", (now(),)).fetchone()[0]
                if count >= 2:
                    return None
