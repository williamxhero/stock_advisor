...
public sealed class LoopbackHttpGateway : ICompanionGateway
{
    ...
    public Task<JsonObject> GetProviderQualityAsync(string view = "summary", IReadOnlyDictionary<string, string>? query = null, CancellationToken cancellationToken = default)
    {
        var resource = view switch { "summary" => "", "comparison" => "/comparison", "errors" => "/errors", _ => throw new ArgumentOutOfRangeException(nameof(view)) };
        return GetAsync($"/v1/provider-quality{resource}{Query(query)}", cancellationToken);
    }
    public Task<string> ExportProviderQualityAsync(string format, IReadOnlyDictionary<string, string>? query = null, CancellationToken cancellationToken = default)
    {
        if (format is not ("json" or "csv")) throw new ArgumentOutOfRangeException(nameof(format));
        var values = query is null ? new Dictionary<string, string>() : new Dictionary<string, string>(query);
        values["format"] = format;
        return GetTextAsync($"/v1/provider-quality/export{Query(values)}", cancellationToken);
    }
