...
from .provider_broker import ProviderBroker, StageRequest
from .web_access_gateway import WebAccessGatewayError
...
class BrokerResearchPlanner:
    """Ask a Provider only for declarative JSON; acquisition stays local."""

    def __init__(self, broker: ProviderBroker, *, deadline: Callable[[], float] | None = None,
    ...
    def __call__(self, packet: dict[str, Any], gaps: list[str], round_number: int) -> dict[str, Any]:
        ...
        planning_packet = {
            ...
            "available_backends": [
                "gateway",
                *(["market"] if packet.get("deterministic_market_facts") else []),
            ],
            "instruction": (
        ...
        encoded = json.dumps(planning_packet, ensure_ascii=False, sort_keys=True, separators=(",", ":")).encode("utf-8")
        request = StageRequest(
            stage="research", packet=planning_packet, packet_sha256=hashlib.sha256(encoded).hexdigest(),
            effort=self.effort, schema=RESEARCH_PLAN_SCHEMA, mode="race", required_capabilities=("race",),
        ...
        if not isinstance(outcome.result, dict):
            raise ResearchPlanError("Provider did not return a qualified research plan")
...
class ReadOnlyResearchExecutor:
    ...
    def __init__(self, backends: dict[str, Callable[[str, dict[str, Any]], dict[str, Any]]]) -> None:
    ...
    def validate_plan(self, plan: dict[str, Any]) -> list[dict[str, Any]]:
        ...
        if not isinstance(operations, list) or len(operations) > 24:
            raise ResearchPlanError("research plan operations must be an array of at most 24 items")
        required = {"requirement_key", "backend", "operation", "arguments", "fallback_backends"}
