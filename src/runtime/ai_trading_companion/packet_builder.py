...
class RuntimePacketBuilder:
    ...
    def prompt(packet: dict[str, Any]) -> str:
        ...
        instruction = {
            ...
            "m0_compose": "只根据冻结 evidence packet（尤其是 market_news_delta）写事实主导的自然市场观察，不调用或补查 article_range。输出严格包含 result_version=3 和 semantic.summary、observations、connections、attention、unknowns；summary 是一个 {text,evidence_refs} 对象，其余每项都是同样结构的对象，evidence_refs 只能逐项填写冻结 packet 中实际存在的 opaque ref。整篇最多提及两只持仓。先讲市场概括，再讲不超过三条新变化、两条关键联系、一条轻度关注和一条真正影响理解的不确定性；正文自然段顺序固定为市场概括→新变化→关键联系→轻度关注→必要不确定性，引用只作内部审计，不能写进正文。不要报幕式逐项复述指数、持仓报价或输入字段；不把缺失字段或采集失败当成正文。若有传言，必须分开表达 truth_status 与 propagation_status：未证实不写成事实，传播不证明真伪；传播广度必须有独立证据，不能从单篇文章推断。calendar_context 优先于记忆；严格禁止方向预测、机会排序、买卖、仓位、加减仓、持有/清仓、操作建议或隐藏的 M1 结论。",
