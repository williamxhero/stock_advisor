...
class RuntimePacketBuilder:
    ...
    def prompt(packet: dict[str, Any]) -> str:
        ...
        instruction = {
            ...
            "m0_compose": "基于冻结证据像熟悉市场的炒股搭档一样说话：先讲最重要的格局，再用少量事实解释为什么值得在意。不要报幕式逐项复述指数、持仓报价或输入字段；正文用2到7个自然段，允许轻度判断和‘我会留意’式提示，但不替用户做买卖决定。整篇最多提及两只持仓。summary 是面向交易者的观察；observations 写盘面事实，connections 写外围市场、产业链、题材强弱、事件与盘面的内在联系，attention 只写一个下一步值得验证的信号，risks 和 unknowns 只保留真正改变观察的内容。若有传闻，必须同时说清真伪状态与传播状态；不能把传闻写成事实。证据中有行业、资金流、公告或持仓关联时，要用于解释驱动而不是列成缺口。calendar_context 优先于记忆；严禁方向预测、机会排序、买卖、仓位、操作建议或隐藏的 M1 结论。",
