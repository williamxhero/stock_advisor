...
class RuntimePacketBuilder:
    ...
    def prompt(packet: dict[str, Any]) -> str:
        display_contract = (
            ...
            "正文只讲经过取舍后真正重要的观察、判断与不确定性。先主动补查；原指标不可得但已有合格代理时，"
            "先交付替代证据揭示的市场状态和交易含义，不得把未取得、未获取或数据缺失当作合格正文。"
            "只有尚未闭合的关键缺口确实阻断方向或动作时，才说明它具体限制了哪项判断，不得罗列采集失败或缺失字段。"
            "如果需要转贴短小外部材料，先用一句自己的话说明为什么值得看，再把材料放进 Markdown 引用块，"
            "并附上可点击的来源链接；材料的列表、表格和强调只属于引用块，不能扩散到自己的话。"
            "材料较长时默认只讲自然摘要并给链接，除非用户明确要原文。"
        )
        instruction = {
            "m0_research": "广泛搜索公开市场信息并输出 Evidence v3 证据剪报。输出 as_of 必须逐字使用 Stage Packet 的 as_of，逐项填写 evidence_contract.requirements 的 coverage。sources、coverage、conflicts 与 high_impact_events 只能引用本轮工具返回的 opaque evidence_ref；source 只能写 evidence_ref、连续原文 excerpt 和分析字段，绝不写 URL、标题、来源身份或任何时间戳。checked_no_change 必须由本轮可追溯的核查结果支持，搜索列表和查询动作本身不能证明无变化。严格遵守冻结窗口；同一原稿的转载不能算独立交叉验证，不得平均不同口径数字。每个 high_impact_event 必须有可跨轮追踪的 event_id，并分别填写 truth_status、propagation_status、truth_evidence_refs、propagation_evidence_refs 与 origin_evidence_refs；若传播已观察到，还必须填写传播观察起止时间；澄清或反证必须填写它所澄清的 clarifies_event_id。未经证实的信息不能冒充事件事实，但传播影响可以作为独立事实。记录实际覆盖和关键失败。可以用 companion_context 调整搜索重点，但只把其中公开股票、题材和事件用于搜索，禁止把账户、成交、身份、路径或其他私密细节写入搜索词。除本包明确提供的内容外，不读取本地文件或用户资料。",
            "m0_compose": "只根据冻结 evidence packet（尤其是 market_news_delta）写事实主导的自然市场观察，不调用或补查 article_range。输出严格包含 result_version=3 和 semantic.summary、facts、derived_metrics、source_opinions、propagation、conflicts、observations、connections、attention、unknowns；summary 与每个公开观察项至少引用一个冻结 packet 中实际存在的 opaque ref。facts 只放市场事实，derived_metrics 只放确定性计算，source_opinions 只放来源观点，propagation 只放传播事实，conflicts 只放冲突证据，unknowns 只放关键未知；每项可带 kind 和必须可验证的 evidence_refs。整篇最多提及两只持仓。先讲市场概括，再讲不超过三条新变化、两条关键联系、一条轻度关注和一条真正影响理解的不确定性；正文自然段顺序固定为市场概括→新变化→关键联系→轻度关注→必要不确定性，引用只作内部审计，不能写进正文。不要报幕式逐项复述指数、持仓报价或输入字段；不把缺失字段或采集失败当成正文。若有传言，必须分开表达 truth_status 与 propagation_status：未证实不写成事实，传播不证明真伪；传播广度必须有独立证据，不能从单篇文章推断。calendar_context 优先于记忆；严格禁止方向预测、机会排序、买卖、仓位、加减仓、持有/清仓、操作建议或隐藏的 M1 结论。",
