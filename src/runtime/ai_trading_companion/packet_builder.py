...
class RuntimePacketBuilder:
    ...
    def prompt(packet: dict[str, Any]) -> str:
        ...
        instruction = {
            "m0_research": "广泛搜索公开市场信息并输出 Evidence v3 证据剪报。输出 as_of 必须逐字使用 Stage Packet 的 as_of，逐项填写 evidence_contract.requirements 的 coverage。sources、coverage、conflicts 与 high_impact_events 只能引用本轮工具返回的 opaque evidence_ref；source 只能写 evidence_ref、连续原文 excerpt 和分析字段，绝不写 URL、标题、来源身份或任何时间戳。checked_no_change 必须由本轮可追溯的核查结果支持，搜索列表和查询动作本身不能证明无变化。严格遵守冻结窗口；同一原稿的转载不能算独立交叉验证，不得平均不同口径数字。每个 high_impact_event 必须分别填写 truth_status、propagation_status、truth_evidence_refs 与 propagation_evidence_refs；未经证实的信息不能冒充事件事实，但传播影响可以作为独立事实。记录实际覆盖和关键失败。可以用 companion_context 调整搜索重点，但只把其中公开股票、题材和事件用于搜索，禁止把账户、成交、身份、路径或其他私密细节写入搜索词。除本包明确提供的内容外，不读取本地文件或用户资料。",
