...
class RuntimePacketBuilder:
    ...
    def prompt(packet: dict[str, Any]) -> str:
        ...
        instruction = {
            ...
            "m0_compose": "基于冻结证据先给出有取舍、有解释力的盘面观察：最重要的格局是什么、强弱在哪里、为什么值得用户在意。事实只选一到两个最能支撑观察的例子，不得逐项复述指数、持仓报价或输入字段。整篇最多提及两只持仓的名称或代码；即使全部持仓都有行情，也不要逐只点评。summary 必须是面向交易者的观察，不得解释 M0、阶段、冻结、工具、确定性投影或内部流程。observations、risks 和 unknowns 只保留会实质改变当前观察的少量内容；没有就留空，不得罗列未请求的数据、常识性盘中 caveat、覆盖口径或‘已检查且无变化’的技术含义。calendar_context 是本地交易日历给出的确定性事实，优先级高于记忆和网页；若历史材料与它冲突，必须把历史材料视为错误或过期信息。盘前交流只能改变关注点，不能替代公开核验，也不能要求 AI 赞同。可以解释此刻盘面偏暖、偏冷、强弱、分化和异常，但为保护随后 H0 与盲 M1 的独立性，严禁给出方向预测、机会排序、买卖、仓位、操作建议或隐藏的 M1 结论。",
            ...
            "m1_judgment": "像独立的专业炒股者一样形成 M1。先在 current_action 说明现在怎么做，再用最多三项 key_evidence 解释主判断。qualified 为 true 时，transition_conditions 的每一项都必须同时填写价格方向、市场广度或相对强弱、持续确认；单个指数短暂越过前收绝不能单独改变方向。position_focus 最多两只，按价格结构、板块相对强弱、仓位和流动性排序，不能用成本或浮亏决定优先级。观望可以是判断但不能含糊；关键证据不足时 direction 与 current_action 必须保守且明确说明本次不应判断。不要提及、猜测或回应 H0。",
