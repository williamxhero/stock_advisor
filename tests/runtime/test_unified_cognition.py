...
class UnifiedCognitionTests(unittest.TestCase):
    ...
    def test_unified_cognition_uses_visible_provider_broker_without_native_search(self) -> None:
        ...
        broker = Mock()
        broker.invoke.return_value = SimpleNamespace(
            winner_route="route-a", result=result, visible_incomplete=False,
        )
        settings = SimpleNamespace(provider={"models": {"fast": {"effort": "medium"}}})

        with patch("ai_trading_companion.__main__.provider_broker", return_value=broker), patch(
        ...
        self.assertEqual(("race",), request.required_capabilities)
