...
from scripts.companion_asr import lexicon
from scripts.companion_runtime.engine import CompanionEngine
from scripts.companion_runtime.governance import RouterGovernance, classify_regime
from scripts.companion_runtime.learning import JudgmentLifecycle, WorkflowEvolution
from scripts.companion_runtime.packet_builder import RuntimePacketBuilder
from scripts.companion_runtime.projection import LearningProjectionRenderer
from scripts.companion_runtime.router import CognitiveRouter
...
class CompanionLearningTests(unittest.TestCase):
    ...
    def test_daily_public_ledger_drives_incremental_mode_and_preserves_gaps(self):
        ...
        packet = RuntimePacketBuilder(PROJECT_ROOT, self.store).build(intraday, "m0_research")
    ...
    def test_all_companion_output_schemas_are_strict_at_every_object(self):
        ...
        for name in names:
            schema = json.loads((PROJECT_ROOT / "automations" / "contracts" / name).read_text(encoding="utf-8"))
            check(schema, name)
