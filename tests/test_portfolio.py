...
from scripts.companion_runtime.portfolio import PortfolioService, explicit_fixture_extraction
...
class PortfolioServiceTests(unittest.TestCase):
    def setUp(self) -> None:
        ...
        (self.root / "data/portfolio").mkdir(parents=True)
        (self.root / "data/logs").mkdir(parents=True)
        (self.root / "data/state").mkdir(parents=True)
        (self.root / "data/portfolio/01_CURRENT_PORTFOLIO.md").write_text(PORTFOLIO, encoding="utf-8")
        ...
        with (self.root / "data/logs/05_DECISION_LOG.csv").open("w", encoding="utf-8", newline="") as handle:
            csv.writer(handle).writerow(headers)
        with (self.root / "data/state/11_STOCK_STATE.csv").open("w", encoding="utf-8", newline="") as handle:
    ...
    def test_complete_buy_updates_projection_markdown_and_log_once(self):
        ...
        self.assertIn("AI Decision Center 成交记录", (self.root / "data/portfolio/01_CURRENT_PORTFOLIO.md").read_text(encoding="utf-8"))
    ...
    def test_explicit_total_assets_correction_updates_account_without_decision_log_trade(self):
        ...
        self.assertNotIn("portfolio_transaction_id", (self.root / "data/logs/05_DECISION_LOG.csv").read_text(encoding="utf-8"))
    ...
    def test_render_failure_reconciles_without_duplicate_log_row(self):
        ...
        log = (self.root / "data/logs/05_DECISION_LOG.csv").read_text(encoding="utf-8")
        self.assertEqual(1, log.count("portfolio_transaction_id="))
        self.assertEqual([], self.service.reconcile())
        self.assertEqual(1, (self.root / "data/logs/05_DECISION_LOG.csv").read_text(encoding="utf-8").count("portfolio_transaction_id="))
    ...
    def test_external_markdown_change_blocks_pending_render(self):
        ...
        self.service.apply_extraction("我以39.5元买入新泉股份100股", self.extraction(), "cycle", "artifact")
        self.service._render_portfolio = original_render
        path = self.root / "data/portfolio/01_CURRENT_PORTFOLIO.md"
